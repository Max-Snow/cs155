...
router.get('/profile', asyncMiddleware(async (req, res, next) => {
  ...
  if(req.query.username != null) { // if visitor makes a search query
    ...
    if(result) { // if user exists
      ...
      render(req, res, next, 'profile/view', 'View Profile', htmlEntities(req.query.username)+" does not exist!", req.session.account);
    }
...
router.get('/transfer', (req, res, next) => {
  ...
  render(req, res, next, 'transfer/form', 'Transfer Bitbars', false, {receiver:null, amount:null, token:generateToken(req.session)});
});
...
router.post('/post_transfer', asyncMiddleware(async(req, res, next) => {
  ...
  if(generateToken(req.session)!=req.body.csrf_token) {
    render(req, res, next, 'transfer/form', 'Transfer Bitbars', 'Invalid transaction! (CSRF)', {receiver:null, amount:null, token:generateToken(req.session)});
    return;
  }

  if(req.body.destination_username === req.session.account.username) {
    render(req, res, next, 'transfer/form', 'Transfer Bitbars', 'You cannot send money to yourself!', {receiver:null, amount:null, token:generateToken(req.session)});
    return;
  }

  const db = await dbPromise;
  
  var query = `SELECT * FROM Users WHERE username == "${req.session.account.username}";`;
  const result = await db.get(query);
  if(req.session.account.hashedPassword != result.hashedPassword) {
    render(req, res, next, 'transfer/form', 'Transfer Bitbars', 'Username and password do not match!', {receiver:null, amount:null, token:generateToken(req.session)});
    return;
  }
  
  var query = `SELECT * FROM Users WHERE username == "${req.body.destination_username}";`;
  const receiver = await db.get(query);
  if(receiver) { // if user exists
    const amount = parseInt(req.body.quantity);
    if(Number.isNaN(amount) || amount > result.bitbars || amount < 1) {
      render(req, res, next, 'transfer/form', 'Transfer Bitbars', 'Invalid transfer amount!', {receiver:null, amount:null, token:generateToken(req.session)});
      return;
    }

    //req.session.account.bitbars -= amount;
    const current_bitbars = result.bitbars - amount;
    query = `UPDATE Users SET bitbars = "${current_bitbars}" WHERE username == "${req.session.account.username}";`;
    await db.exec(query);
    ...
    await db.exec(query);
    query = `SELECT * FROM Users WHERE username == "${req.session.account.username}";`;
    const new_result = await db.get(query);
    req.session.account = new_result;
    ...
    render(req, res, next, 'transfer/form', 'Transfer Bitbars', `User ${q} does not exist!`, {receiver:null, amount:null, token:generateToken(req.session)});
  }
}));
