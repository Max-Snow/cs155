			...
			if tcpLayer != nil && ipLayer != nil && etherLayer != nil {
				...
				*/
				...
				if tcp.SYN == true && tcp.ACK == false {
					if val, ok := addresses[ip.SrcIP.String()]; ok {
						val[0] += 1
						addresses[ip.SrcIP.String()] = val
					} else {
						val := [2]int{1, 0}
						addresses[ip.SrcIP.String()] = val
					}
				} else if tcp.SYN == true && tcp.ACK == true {
					if val, ok := addresses[ip.DstIP.String()]; ok {
						val[1] += 1
						addresses[ip.DstIP.String()] = val
					} else {
						val := [2]int{0, 1}
						addresses[ip.DstIP.String()] = val
					}
				...
				*/
				...
				if arp.Operation == 1 {
					...
					source_ip := net.IP(arp.SourceProtAddress).String()
					arpRequests[source_ip] = map[string]int{}
				} else if arp.Operation == 2 {
					// Write code for handling an ARP reply
					source_ip := net.IP(arp.SourceProtAddress).String()
					source_mac := net.HardwareAddr(arp.SourceHwAddress).String()
					if val, ok := arpRequests[source_ip]; ok {
						if len(val) > 0 {
							arpMac[source_mac] += 1
						}
						val[source_mac] += 1
